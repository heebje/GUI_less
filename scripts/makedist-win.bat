...
if %1 == 1 (echo Making GUI_less Windows DEMO VERSION distribution ...) else (echo Making GUI_less Windows FULL VERSION distribution ...)
...
REM - msbuild GUI_less-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild GUI_less.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild GUI_less.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\GUI_less.aaxplugin\Contents\Win32\GUI_less.aaxplugin --out .\build-win\aax\bin\GUI_less.aaxplugin\Contents\Win32\GUI_less.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\GUI_less.aaxplugin\Contents\x64\GUI_less.aaxplugin --out .\build-win\aax\bin\GUI_less.aaxplugin\Contents\x64\GUI_less.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\GUI_less.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\GUI_less.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "GUI_less Installer" ".\installer\GUI_less Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\GUI_less Installer.exe" ".\installer\GUI_less Demo Installer.exe"
REM -del ".\installer\GUI_less Installer.exe"
REM -)
