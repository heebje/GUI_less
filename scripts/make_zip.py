...
def  main():
  ...
  installer = "\installer\GUI_less Installer.exe"
   
  if demo:
    installer = "\installer\GUI_less Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\GUI_less manual.pdf" 
  ]
  ...
  ZIPNAME = "GUI_less-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "GUI_less-v" + FULLVERSIONSTR + "-win-demo.zip"
